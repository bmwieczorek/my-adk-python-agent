...
| File                                                 | Purpose                                                                                         |
|------------------------------------------------------|-------------------------------------------------------------------------------------------------|
| `requirements-docker.in` → `requirements-docker.txt` | Lean deps for running in Docker/Podman (ADK FastAPI runtime / A2A) — `google-adk` + `pendulum` + `a2a-sdk` |
...
# Model C (integrated A2A via FastAPI runtime):
...
|----------|---------|-------------|
| `SERVE_MODE` | `adk` | `adk` for the unified ADK FastAPI runtime, `a2a` for standalone A2A JSONRPC server |
| `A2A` | `false` | When `true` with `SERVE_MODE=adk`, enables integrated ADK FastAPI A2A routes (Model C) |
| `A2A_AGENT_MODULE` | `my_multi_agent` | Agent folder name used for both standalone and integrated A2A modes |
| `FASTAPI_APP_MODULE` | `adk_fastapi_server:app` | Module path for uvicorn when `SERVE_MODE=adk` |
| `TRACE_TO_CLOUD` | *(unset)* | Set to `true` to enable Cloud Trace export in the unified FastAPI runtime |
| `A2A_CARD_BASE_URL` | `http://localhost:${HOST_PORT}` | Base URL used to generate Model C `agent.json` card URL |
| `A2A_CARD_PATH_PREFIX` | `/a2a` | URL prefix used for Model C routes and generated card URL |
...
### Three A2A implementation models (important)

The terms "A2A enabled" can refer to different runtime models. Do not mix
...
| **Model C — framework-integrated A2A via ADK FastAPI runtime** | `uvicorn adk_fastapi_server:app` with `A2A=true` | framework/version dependent (in ADK 1.31, `/a2a/{app}/.well-known/agent-card.json`) | framework/version dependent (in ADK 1.31, `POST /a2a/{app}` with JSON-RPC) | in this repo: `SERVE_MODE=adk`, `A2A=true`, `A2A_AGENT_MODULE` (+ optional card URL vars); runtime uses `get_fast_api_app(..., a2a=True)` |

This repository supports **Model A and Model C**. Model B is included for
cross-runtime comparison.

#### Requirements checklist by model

| Requirement | Model A (standalone `to_a2a`) | Model B (`adk web` integrated) | Model C (FastAPI integrated) |
|---|---|---|---|
| Runtime entrypoint | `uvicorn a2a_server:app` | `adk web` + runtime-specific A2A settings | `uvicorn adk_fastapi_server:app` |
| Minimal settings | `SERVE_MODE=a2a`, `A2A_AGENT_MODULE` | runtime-specific (often `A2A=true`, `A2AConfig`, app-local card setup) | `SERVE_MODE=adk`, `A2A=true`, `A2A_AGENT_MODULE` |
| Additional settings commonly used | `A2A_PROTOCOL`, `A2A_HOST`, `A2A_PORT` | framework/version specific | `FASTAPI_APP_MODULE`, `A2A_CARD_BASE_URL`, `A2A_CARD_PATH_PREFIX`, `TRACE_TO_CLOUD` |
| Agent card source | auto-generated by `to_a2a(root_agent, ...)` | framework/runtime specific | `<app>/agent.json` (generated by `prepare_adk_web_a2a_agent_card.py`) |
| Discovery endpoint | `GET /.well-known/agent-card.json` | commonly `GET /.well-known/agent.json` (or runtime-specific) | `GET /a2a/{app}/.well-known/agent-card.json` (ADK 1.31 shape) |
| JSON-RPC endpoint | `POST /` | commonly `/on_message_send*` or runtime-prefixed routes | `POST /a2a/{app}` (ADK 1.31 shape) |
| Best fit | strict/stable standalone A2A endpoint | runtimes that expose A2A through `adk web` integration | ADK web/API + A2A in one FastAPI runtime |

#### Pros/cons and deployment shape by model

| Model | Pros | Cons | Sidecar usage | One container with ADK web + A2A on same base URL (suffix) | Example URLs (web + A2A) |
|---|---|---|---|---|---|
| **Model A** | Most explicit A2A contract (`/.well-known/agent-card.json`, `POST /`); easy to validate with A2A tools | Separate runtime from ADK web UX/API; to have both, use sidecar (or separate services) | **Used in this repo's GKE sidecar pattern** (`adk` container + `a2a` container) | Not native in one container/base URL for Model A | `Web (sidecar host): https://bartek-adk-agent-<namespace>.<cluster-subdomain>.us-central1.<domain>/list-apps`<br>`A2A card (sidecar host): https://bartek-adk-agent-a2a-<namespace>.<cluster-subdomain>.us-central1.<domain>/.well-known/agent-card.json`<br>`A2A RPC: POST https://bartek-adk-agent-a2a-<namespace>.<cluster-subdomain>.us-central1.<domain>/` |
| **Model B** | Single runtime can expose web + A2A together in some frameworks | Highest framework/version variance in routes and config shape | Optional, usually not needed | Often yes, if that framework's integrated A2A supports prefixed routes | `Web: https://bartek-adk-agent-<masked-number>.us-central1.run.app/list-apps`<br>`A2A card: https://bartek-adk-agent-<masked-number>.us-central1.run.app/.well-known/agent.json`<br>`A2A RPC: POST https://bartek-adk-agent-<masked-number>.us-central1.run.app/on_message_send` |
| **Model C** | Single container in this repo; serves ADK web/API plus A2A under `/a2a/{app}`; clean Cloud Run fit | Still framework/version dependent; requires `A2A=true` and `agent.json` generation flow | Optional, not required | **Yes (native in this repo)** via `SERVE_MODE=adk` + `A2A=true` | `Web: https://bartek-adk-agent-<masked-number>.us-central1.run.app/list-apps`<br>`A2A card: https://bartek-adk-agent-<masked-number>.us-central1.run.app/a2a/my_multi_agent/.well-known/agent-card.json`<br>`A2A RPC: POST https://bartek-adk-agent-<masked-number>.us-central1.run.app/a2a/my_multi_agent` |

> Sidecar is a deployment pattern, not an A2A protocol requirement. In this repo,
> use sidecar when you want **ADK web + Model A** together.

#### Gemini Enterprise registration/chat compatibility

| Model | Gemini Enterprise support |
|---|---|
| **Model A** | **Yes**, when endpoint is HTTPS-reachable, IAM allows Discovery Engine invoker, and the A2A card/JSON-RPC endpoint are valid |
| **Model B** | **Depends** on runtime/version; works only if that integration is fully A2A-compliant |
| **Model C** | **Yes (confirmed by working example)** |

Requirements for all models:
1. HTTPS-reachable endpoint.
2. Cloud Run IAM allows Gemini Enterprise caller identity (Discovery Engine service agent).
3. Valid A2A card and callable message endpoint consistent with the card.
...
# Model B common patterns:
curl -si "${BASE_URL}/.well-known/agent.json" | head -n 1
curl -si "${BASE_URL}/on_message_send" | head -n 1
# Model C (ADK 1.31 path shape):
curl -si "${BASE_URL}/a2a/<app>/.well-known/agent-card.json" | head -n 1
```

If `/.well-known/agent-card.json` is `200` with root JSON-RPC at `/`, you are on
Model A. If `/.well-known/agent.json` / `/on_message_send*`-style routes are
active, you are on Model B. If `/a2a/<app>/...` routes are active, you are on
Model C.
...
|---|---|---|
| `adk` (default) | `uvicorn adk_fastapi_server:app --host 0.0.0.0 --port ${PORT:-APP_PORT}` | Unified ADK FastAPI runtime, optionally with integrated Model C A2A routes when `A2A=true` |
...
### How Model C works in this repo
...
2. Starts `uvicorn adk_fastapi_server:app`, and that runtime calls
   `get_fast_api_app(..., a2a=True)` so ADK mounts framework-managed A2A routes.
...
| `prepare_adk_web_a2a_agent_card.py` | Runtime helper that writes `<app>/agent.json` for Model C |
| `adk_fastapi_server.py` | Unified FastAPI entrypoint using `get_fast_api_app(..., a2a=A2A)` |
| `my_multi_agent/agent.py` | Agent definition — `root_agent` with tools and instructions |
| `Dockerfile` | Conditional `CMD` — runs `uvicorn adk_fastapi_server:app` or `uvicorn a2a_server:app` based on `SERVE_MODE` |
...
Model C (integrated FastAPI runtime):
...
uvicorn adk_fastapi_server:app --host 0.0.0.0 --port 8000
```
...
For Model C (integrated A2A via the FastAPI runtime), keep `SERVE_MODE=adk` and
enable `A2A=true`:
...
|----------|---------|-------------|
| `SERVE_MODE` | `adk` | `adk` for the unified ADK FastAPI runtime, `a2a` for standalone A2A JSONRPC server |
| `A2A` | `false` | When `true` with `SERVE_MODE=adk`, enables integrated ADK FastAPI A2A routes |
| `A2A_AGENT_MODULE` | `my_multi_agent` | Agent folder name used for both standalone and integrated A2A modes |
| `FASTAPI_APP_MODULE` | `adk_fastapi_server:app` | Module path for uvicorn when `SERVE_MODE=adk` |
...
| `A2A_CARD_BASE_URL` | *(unset)* | Optional base URL used to generate Model C `agent.json` (defaults to protocol+host+APP_PORT) |
| `A2A_CARD_PATH_PREFIX` | `/a2a` | URL prefix used by Model C route/card generation |
| `TRACE_TO_CLOUD` | *(unset)* | Set to `true` to enable Cloud Trace export in the unified FastAPI runtime |
...
This sidecar deployment corresponds to:
- ADK web/user-facing runtime in the `adk` container
- **Model A** standalone A2A in the `a2a` container

If you want one container and one base URL with A2A under a suffix, use
**Model C** (`SERVE_MODE=adk`, `A2A=true`) instead of sidecar.
...
# Model C for Gemini Enterprise registration (integrated FastAPI A2A):
# export SERVE_MODE=adk
# export A2A=true
# export A2A_AGENT_MODULE=my_multi_agent
...
- In adk mode, runs the unified FastAPI runtime (`uvicorn adk_fastapi_server:app`)
  with `A2A=true` for integrated A2A routes.
- Passes through optional reference-style runtime vars when set:
  `TRACE_TO_CLOUD`, `ADK_DISABLE_LOCAL_STORAGE`, `MEMORY_SERVICE_URI`,
  `SESSION_SERVICE_URI`, `BQ_ANALYTICS_PLUGIN_ENABLE`.
- In Model A (`SERVE_MODE=a2a`), updates `A2A_HOST` to the deployed Cloud Run hostname.
- In Model C (`SERVE_MODE=adk` + `A2A=true`), updates `A2A_CARD_BASE_URL` to the deployed Cloud Run URL.
- Prints endpoint URLs for the selected mode.

### RCA: Gemini Enterprise `403 PERMISSION_DENIED` (`REMOTE_AGENT_FAILURE`)

#### Symptom

Gemini Enterprise registration/chat fails with:
...
Cloud Run denied invocation before request reached the agent app.
The service was private and only a human user had `roles/run.invoker`, while Gemini
Enterprise invoked as a **Discovery Engine service agent**.

#### Why this happened

1. Unauthenticated calls to Cloud Run returned 403.
2. Authenticated developer-token calls returned 200.
3. The working reference service had `roles/run.invoker` assigned to:
   `serviceAccount:service-<DISCOVERY_ENGINE_PROJECT_NUMBER>@gcp-sa-discoveryengine.iam.gserviceaccount.com`.

#### Correct fix (recommended for enterprise org policies)

Grant `roles/run.invoker` to the Discovery Engine service agent used by Gemini Enterprise:

```bash
PROJECT_ID=<cloud-run-project>
REGION=<cloud-run-region>
SERVICE_NAME=<cloud-run-service>
DISCOVERY_ENGINE_PROJECT_NUMBER=<project-number-used-by-gemini-enterprise>

DISCOVERY_ENGINE_INVOKER_SA="serviceAccount:service-${DISCOVERY_ENGINE_PROJECT_NUMBER}@gcp-sa-discoveryengine.iam.gserviceaccount.com"

gcloud run services add-iam-policy-binding "${SERVICE_NAME}" \
  --project "${PROJECT_ID}" \
  --region "${REGION}" \
  --member "${DISCOVERY_ENGINE_INVOKER_SA}" \
...
#### Important note about `allUsers`

If your org blocks public exposure, `allUsers` binding fails by policy.
In that case, use service-agent IAM bindings (as above), not public invoker access.

#### Applies to Model A, Model B, and Model C

This IAM requirement is **independent of A2A model**. All models need Cloud Run
invocation permissions aligned with the caller identity.
...
```bash
# Requires:
# GOOGLE_CLOUD_PROJECT and GOOGLE_CLOUD_LOCATION are already exported in your shell.
CLOUD_RUN_SERVICE_NAME="bartek-adk-agent"

# Prefer the numeric+region run.app URL when Cloud Run returns multiple URLs.
CLOUD_RUN_SERVICE_URL="$(
  ...
  | jq -r '
      ...
      | fromjson
      | map(select(test("-[0-9]+\\.[a-z0-9-]+\\.run\\.app$")))[0] // empty
    '
)"
...
curl -s -H "Authorization: Bearer $(gcloud auth print-identity-token)" \
  "${CLOUD_RUN_SERVICE_URL}/.well-known/agent-card.json" | jq

# Model C (my_multi_agent example):
curl -s -H "Authorization: Bearer $(gcloud auth print-identity-token)" \
  "${CLOUD_RUN_SERVICE_URL}/a2a/my_multi_agent/.well-known/agent-card.json" | jq
```

If you want to reuse one token across multiple requests (Model C card + `list-apps`):
...
# ADK web/API apps list
curl -s -H "Authorization: Bearer ${ID_TOKEN}" \
  "${CLOUD_RUN_SERVICE_URL}/list-apps" | jq
```

The JSON responses are the same as calling the equivalent proxied local URLs
(`http://127.0.0.1:8080/...`), so output is not duplicated here.
...
# Model C Agent Card (my_multi_agent example)
http://127.0.0.1:8080/a2a/my_multi_agent/.well-known/agent-card.json

# 1) Via local Cloud Run proxy (no explicit token needed in curl)
curl -s http://127.0.0.1:8080/a2a/my_multi_agent/.well-known/agent-card.json | jq
```

Example output for the command above (hash segment masked):
...
```json
{
  ...
  "description": "A2A endpoint for ADK app 'my_multi_agent'.",
  "name": "my_multi_agent",
  "preferredTransport": "JSONRPC",
  "protocolVersion": "0.3.0",
  "skills": [
    {
      "description": "Interact with ADK app 'my_multi_agent'.",
      "id": "my_multi_agent",
      "name": "my_multi_agent",
      "tags": [
        "adk",
  ...
  "url": "https://bartek-adk-agent-<masked>.run.app/a2a/my_multi_agent",
  "version": "1.0.0"
}
```

```bash
# ADK web UI (depending ADK/runtime version, root may redirect to /dev-ui/)
http://127.0.0.1:8080
http://127.0.0.1:8080/dev-ui/

# List available apps via ADK web/API
curl -s http://127.0.0.1:8080/list-apps | jq

# Filter apps that start with "my_" and end with "_agent"
curl -s http://127.0.0.1:8080/list-apps | jq '[.[] | select(startswith("my_") and endswith("_agent"))]'
...
This repo supports Model A and Model C from
[Three A2A implementation models (important)](#three-a2a-implementation-models-important):

- **Model C (integrated):** `SERVE_MODE=adk` + `A2A=true` → `uvicorn adk_fastapi_server:app`
- **Model A (standalone):** `SERVE_MODE=a2a` → `uvicorn a2a_server:app`

#### Discover the Agent Card

```bash
# Model C (integrated FastAPI, app=my_multi_agent):
