...
# Model B (integrated A2A via FastAPI runtime):
...
| `A2A` | `false` | When `true` with `SERVE_MODE=adk`, enables integrated ADK FastAPI A2A routes (Model B) |
...
| **Model B — framework-integrated A2A via ADK FastAPI runtime** | `uvicorn adk_fastapi_server:app` with `A2A=true` | framework/version dependent (in ADK 1.31, `/a2a/{app}/.well-known/agent-card.json`) | framework/version dependent (in ADK 1.31, `POST /a2a/{app}` with JSON-RPC) | in this repo: `SERVE_MODE=adk`, `A2A=true`, `A2A_AGENT_MODULE` (+ optional card URL vars); other runtimes may use `A2AConfig` |

This repository now supports **both models**.

#### Requirements checklist by model

| Requirement | Model A (standalone `to_a2a`) | Model B (framework-integrated via FastAPI) |
|---|---|---|
| Runtime entrypoint | `uvicorn a2a_server:app` | `uvicorn adk_fastapi_server:app` |
| Minimal env vars | `SERVE_MODE=a2a`, `A2A_AGENT_MODULE` | `SERVE_MODE=adk`, `A2A=true`, `A2A_AGENT_MODULE` |
| Additional env vars commonly used | `A2A_PROTOCOL`, `A2A_HOST`, `A2A_PORT` | `FASTAPI_APP_MODULE`, `A2A_CARD_BASE_URL`, `A2A_CARD_PATH_PREFIX`, `TRACE_TO_CLOUD` |
| Agent card source | auto-generated by `to_a2a(root_agent, ...)` | `<app>/agent.json` (generated by `prepare_adk_web_a2a_agent_card.py`) |
| Discovery endpoint | `GET /.well-known/agent-card.json` | `GET /a2a/{app}/.well-known/agent-card.json` (ADK 1.31 shape) |
| JSON-RPC endpoint | `POST /` | `POST /a2a/{app}` (ADK 1.31 shape) |
| Best fit | strict/stable standalone A2A endpoint | ADK web/API + A2A in one FastAPI runtime |

> In this repository, **Model B means FastAPI-integrated A2A** (`get_fast_api_app(..., a2a=True)`).
...
|---|---|---|
| `adk` (default) | `uvicorn adk_fastapi_server:app --host 0.0.0.0 --port ${PORT:-APP_PORT}` | Unified ADK FastAPI runtime, optionally with integrated Model B A2A routes when `A2A=true` |
...
| `prepare_adk_web_a2a_agent_card.py` | Runtime helper that writes `<app>/agent.json` for Model B |
...
Model B (integrated FastAPI runtime):
...
For Model B (integrated A2A via the FastAPI runtime), keep `SERVE_MODE=adk` and
enable `A2A=true`:
...
# Model B for Gemini Enterprise registration (integrated FastAPI A2A):
# export SERVE_MODE=adk
# export A2A=true
# export A2A_AGENT_MODULE=my_multi_agent
...
#### Applies to both Model A and Model B

This IAM requirement is **independent of A2A model**. Model A and Model B both
need Cloud Run invocation permissions aligned with the caller identity.
...
- **Model B (integrated):** `SERVE_MODE=adk` + `A2A=true` → `uvicorn adk_fastapi_server:app`
- **Model A (standalone):** `SERVE_MODE=a2a` → `uvicorn a2a_server:app`

#### Discover the Agent Card

```bash
# Model B (integrated FastAPI, app=my_multi_agent):
